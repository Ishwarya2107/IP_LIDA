...
app.post("/submit", async (req, res) => {
  ...
  app.post('/loginuser', async (req, res) => {
    ...
    try {
      // Step 1: Create a connection to the MainDB
      const connection = mongoose.createConnection(`mongodb://localhost:27017/MainDB`
    
      );
  
      // Step 2: Define the User schema and model for this connection
      const UserSchema = new mongoose.Schema({
        name: String,
        password: String,
      });
      ...
      // Step 3: Query the user from the database
  ...
  async function importCsvToMongoDB(filePath, dbName, collectionName) {
  ...
  app.get('/export-csv', async (req, res) => {
    ...
    try {
        await exportCollectionsToCSV(database); // Reuse your server-side function
        res.status(200).send('CSV files exported successfully');
    } catch (error) {
        console.error(error);
        res.status(500).send('Error exporting collections to CSV');
    }
});
  ...
  console.log(csvFilePath);
    ...
    try {
      await importCsvToMongoDB(csvFilePath, schoolname, collectionName);
  ...
  app.post("/mark-absentees", async (req, res) => {
    ...
    try {
      ...
      // Step 1: Mark absentees as "Absent"
      ...
      // Step 2: Mark all others as "Present"
      await collection.updateMany(
        { name: { $nin: absentees } }, // Exclude absentees
        {
          $set: {
      ...
      // Step 3: Calculate Total Classes Dynamically
      ...
      students.forEach(student => {
        ...
        Object.keys(attendanceData).forEach(dateKey => {
          if (attendanceData[dateKey][subject]) {
            uniqueDates.add(dateKey); // Add each unique date for the subject
          }
        });
      });
      ...
      // Step 4: Update subject attendance percentage for all students
  ...
  async function exportCollectionsToCSV(databaseName) {
    try {
      ...
      for (const collectionInfo of collections) {
        ...
        // Add collection data to the combined array
      ...
      // Define CSV writer for a single file
